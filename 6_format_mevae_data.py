...
#                         nargs='?', default="3")
    ...
    #     path_parts = os.path.normpath(dir_path).split(os.sep)
    #     for rot_img_path in tqdm(glob(os.path.join(args.cells_dir, "images", "gaussian_{}".format(args.gaussian),
    #                                           dir_path,
    #                                           "rotated_*.tif"))):
    #         idx = rot_img_path[:-4].split("_")[-1]
    #
    #         # ROTATED IMAGE
    #         rot_img = tifffile.imread(rot_img_path)
    #
    #         # ORIGINAL IMAGE
    #         img_path = os.path.join(args.cells_dir, "images", "gaussian_{}".format(args.gaussian), dir_path,
    #                                 "original_{}.tif".format(idx))
    #         img = tifffile.imread(img_path)
    #
    #         # ROTATED NORMALIZED IMAGE
    #         rot_norm_path = os.path.join(args.cells_dir, "images", "gaussian_{}".format(args.gaussian), dir_path,
    #                                      "rotated_normalized_{}.tif".format(idx))
    #
    #         # ROTATED SEGMENTATION
    #         rot_seg_path = os.path.join(args.cells_dir, "segmentations", "gaussian_{}".format(args.gaussian), dir_path,
    #                                     "rotated_{}.tif".format(idx))
    #         rot_seg = tifffile.imread(rot_seg_path)
    #
    #         # ORIGINAL SEGMENTATION
    #         original_seg_path = os.path.join(args.cells_dir, "segmentations", "gaussian_{}".format(args.gaussian),
    #                                          dir_path,
    #                                          "original_{}.tif".format(idx))
    #         seg = tifffile.imread(original_seg_path)
    #
    #         gt_img = transform_image(rot_img, rot_seg)
    #         enc1_img = transform_image(img, seg)
    #
    #         out_name = "{}_{}_label{}.tif".format(path_parts[0], path_parts[1], idx)
    #
    #         enc1_dir = os.path.join(args.out_dir, "gaussian_{}".format(args.gaussian), "MEVAEInputs1", "train")
    #         enc2_dir = os.path.join(args.out_dir, "gaussian_{}".format(args.gaussian), "MEVAEInputs2", "train")
    #         outputs_dir = os.path.join(args.out_dir, "gaussian_{}".format(args.gaussian), "MEVAEOutputs", "train")
    #
    #         os.makedirs(enc1_dir, exist_ok=True)
    #         os.makedirs(enc2_dir, exist_ok=True)
    #         os.makedirs(outputs_dir, exist_ok=True)
    #
    #         shutil.copy(rot_norm_path, os.path.join(enc2_dir, out_name))
    #         tifffile.imwrite(os.path.join(enc1_dir, out_name), enc1_img)
    #         tifffile.imwrite(os.path.join(outputs_dir, out_name), gt_img)
    #
